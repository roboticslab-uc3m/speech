...
bool PiperSynthesizer_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="model")
    {
        paramValue = m_model;
    ...
    if (paramName =="modelConfig")
    {
        paramValue = m_modelConfig;
        return true;
    }
    if (paramName =="eSpeakDataDir")
    {
        paramValue = m_eSpeakDataDir;
        return true;
    }
    if (paramName =="speakerId")
    {
        paramValue = std::to_string(m_speakerId);
        return true;
    }
    if (paramName =="noiseScale")
    {
        paramValue = std::to_string(m_noiseScale);
        return true;
    }
    if (paramName =="lengthScale")
    {
        paramValue = std::to_string(m_lengthScale);
        return true;
    }
    if (paramName =="noiseW")
    {
        paramValue = std::to_string(m_noiseW);
        return true;
    }
...
std::string PiperSynthesizer_ParamsParser::getConfiguration() const
{
